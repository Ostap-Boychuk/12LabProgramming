...
// This function contains the main game loop which controls the game. 
{
	...
	sf::Clock clock;
	myPlayerTwo.setPosition(300, 400);
...
// handles input in the game
{
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::F))
	{
		...
		myPlayer.speedIncrease();
		myPlayerTwo.speedIncrease();
	}

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::S))
	{
		// the player’s movement speed should decrease by 1.
		myPlayer.speedDecrease();
		myPlayerTwo.speedDecrease();
	}

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::I))
	{
		// the size of the player's image increases. This occurs only once.
		myPlayer.scalePlayerSize();
		myPlayerTwo.scalePlayerSize();
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::C))
	{
		...
		myPlayer.colorChange();
		myPlayerTwo.colorChange();
	}
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::D))
	{
		...
		myPlayerTwo.changeDirection();
	}

	// calls the player object to move itself in the appropriate direction
	myPlayer.movementCheck();
	myPlayerTwo.movementCheck();
}
...
// draws/renders the game world
{
	...
	window.draw(myPlayerTwo.getBody());  // draw the player object
